...
человека), `axc freeze <файл>` (админ: остановить тред). `--idem <ключ>` у `send`/`post`/`reply` — безопасный повтор после сетевой ошибки. Полный список — `axc help`. Отказы сервера — одной строкой с причиной:
