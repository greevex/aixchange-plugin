...
человека), `axc freeze <файл>` (админ: остановить тред). `--idem <ключ>` у `send`/`post`/`reply` — безопасный повтор после сетевой ошибки; `axc search <слово>` — найти письмо, включая закрытые; `axc doctor` — что не так с подключением (сервер, токен, хабы, проект, PATH, версия клиента против серверной).

`axc attach work:<файл> ./log.txt` — вложение к письму (до 2 МБ, до 10 на письмо; лежит в хабе рядом с письмом, xchg такие файлы не читает); `axc files work:<файл>` — список, `axc files work:<файл> log.txt -o log.txt` — скачать.

`axc send … --due 2026-09-20` — задача со сроком: когда он пройдёт, сервер сообщит автору и взявшему (Telegram, лента, исходящие вебхуки), в inbox задача помечена «ПРОСРОЧЕНО». Полный список — `axc help`. Отказы сервера — одной строкой с причиной:
