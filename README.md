...
человека), `axc freeze <файл>` (админ: остановить тред). `--idem <ключ>` у `send`/`post`/`reply` — безопасный повтор после сетевой ошибки; `axc search <слово>` — найти письмо, включая закрытые; `axc doctor` — что не так с подключением (сервер, токен, хабы, проект, PATH). Полный список — `axc help`. Отказы сервера — одной строкой с причиной:
